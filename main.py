...
# ---
# Expander mostra os resultados do diagnóstico que já foi feito
...
# Cabeçalho completo centralizado
...
if os.path.exists("logo_finai.png"):
    st.image("logo_finai.png", width=120)
else:
    st.write("*(Logo FinAI)*")
st.markdown("<h1>FinAI Insights</h1>", unsafe_allow_html=True)
st.markdown("</div>", unsafe_allow_html=True) 
st.markdown("<h3>Análise Financeira por IA</h3>", unsafe_allow_html=True)
st.markdown("Analise rapidamente relatórios financeiros com o poder da IA.")
st.markdown("</div>", unsafe_allow_html=True) 

st.markdown("---")

st.subheader("Carregue seus Relatórios Financeiros (PDF)")
...
if uploaded_files:
    if not embeddings:
        ...
        if chunks:
            ...
            # AQUI ESTÁ A MUDANÇA: Aumentando 'k' para 10
    ...
    st.info("⬆️ Por favor, carregue um ou mais relatórios financeiros em PDF para começar a análise.")


if vector_store and llm:
    st.subheader("Pergunte aos Seus Relatórios Financeiros")
    query = st.text_area(
